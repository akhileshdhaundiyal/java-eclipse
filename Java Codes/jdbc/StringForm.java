package jdbc;

import java.util.Date;

public class StringForm 
{
	public static void main(String[] args) 
	{
		System.out.printf("Integer : %d\n", 15);
		System.out.printf("Floating point number with 3 decimal digits: %.3f\n", 1.51312939123);
		System.out.printf("String: %s, integer: %d. float: %.6f \n", "Hello World", 89,9.231435);
		
		System.out.printf("%-12s%-12s%s\n", "Column 1","Column 2", "Column 3");
		System.out.printf("%-12d%-12d%07d\n", 15,12,5);
		
		System.out.printf("*********Floating Point Formatting********\n");
		System.out.printf("%-12s%-12s\n", "Column 1","Column 2");		
		System.out.printf("%-12.5f%.20f \n", 12.23429837, 9.1021304354);
		
		String s = String.format("%-12.5f%.20f", 12.324576287321, 9.10348294812);
		System.out.println(s);
		
		Date d = new Date();
		System.out.println(d.toString());
		System.out.format("Today is %TF %TT", d, d);		
	}
}